  ...
  return (
    <div className="space-y-2">
      ...
      <div className="rounded-2xl p-5 mb-2" style={{
        background: 'linear-gradient(158deg, #3A332B 0%, #3B2A1F 100%)',
        color: '#F2ECE0',
      }}>
        {/* Avatar */}
        <div className="w-12 h-12 rounded-full flex items-center justify-center font-sans font-semibold text-base mb-3"
          style={{ background: 'linear-gradient(152deg, #E8D3BE, #C2A982)', color: '#3B2A1F', fontSize: 18 }}>
        ...
        {/* Tier badge */}
        <span className="inline-flex items-center gap-1.5 text-[12px] font-sans font-semibold px-2.5 py-1 rounded-full"
          ...
          return (
            <Link key={href} href={href}
              ...
              style={active
                ...
                : { color: '#3B2A1F' }}>
              <Icon size={15} style={{ color: active ? '#E8C5A0' : '#675A4E', flexShrink: 0 }} />
              {label}
            </Link>
          );
        })}
      ...
      <div className="pt-1">
        <form action="/api/auth/signout" method="POST">
          <button type="submit"
            ...
            style={{ border: '1px solid rgba(44,36,32,.12)', color: '#675A4E' }}
            onMouseEnter={e => {
              (e.currentTarget as HTMLElement).style.borderColor = '#C4714A';
              (e.currentTarget as HTMLElement).style.color = '#C4714A';
            }}
            onMouseLeave={e => {
              (e.currentTarget as HTMLElement).style.borderColor = 'rgba(44,36,32,.12)';
              (e.currentTarget as HTMLElement).style.color = '#675A4E';
            }}>
