      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="font-serif text-lg text-zen-bark">
            Votre rituel
            {itemCount > 0 && (
              <span className="ml-2 font-sans text-sm text-zen-muted font-normal">
                ({itemCount})
              </span>
            )}
          </h2>
          <button
            onClick={closeCart}
            className="p-1.5 rounded-full text-zen-muted hover:bg-gray-100 hover:text-zen-bark transition-colors"
            aria-label="Fermer"
          >
            <X size={18} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {items.length === 0 && (
            <div className="flex flex-col items-center justify-center h-64 gap-4 px-5 text-center">
              ...
              <p className="text-zen-muted font-sans text-sm">
                Votre rituel est vide pour l’instant.
              </p>
              <button
                ...
                <div key={product.id} className="flex gap-3 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-sans text-sm text-zen-bark leading-snug line-clamp-2">
                        {product.nameFr}
                      </p>
                      <button
                    ...
                    <div className="flex items-center justify-between mt-2.5">
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg">
                        ...
                        <span className="text-sm font-sans text-zen-bark w-4 text-center">
                          {quantity}
                        </span>
                        <button
                          onClick={() => updateQuantity(product.id, quantity + 1)}
              ...
              <div className="divide-y divide-gray-50">
                {upsells.map(product => (
                  <div key={product.id} className="flex items-center gap-3 px-5 py-3 bg-gray-50/60">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-sans text-xs text-zen-bark leading-snug line-clamp-1">
                        {product.nameFr}
                      </p>
