    ...
    <div className="group lift rounded-2xl overflow-hidden cursor-pointer"
      ...
      <div className={`relative aspect-[3/4] overflow-hidden ${hasProductImage ? 'bg-white' : placeholderBg}`}>
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
          ...
          {isVeryLow && (
            <span className="text-[10px] font-sans font-semibold px-2 py-0.5 rounded"
              style={{ background: '#FEF3F0', color: '#9B3D1A', border: '1px solid #FECDB9' }}>
              ⚡ Presque épuisé
            </span>
          )}
          {isLow && !isVeryLow && (
          ...
          {isOutOfStock && (
            <span className="text-[10px] font-sans font-medium px-2 py-0.5 rounded bg-gray-200 text-gray-500">
              Rupture
            </span>
          )}
        </div>
      ...
      <div className="p-5 lg:p-6">
        <p className="text-[10px] font-sans uppercase tracking-widest mb-2" style={{ color: '#9a8878', letterSpacing: '0.08em' }}>
          {product.category?.replace('-', ' & ')}
        </p>
        <Link href={`/${locale}/produits/${product.slug}`}>
          <h3 className="font-serif leading-snug hover:opacity-70 transition-opacity line-clamp-2 mb-3"
            style={{ fontSize: 'clamp(15px, 1.2vw, 17px)', color: '#2C2420' }}>
            {product.nameFr}
          </h3>
        </Link>
        ...
        <div className="flex items-center gap-1 mb-4">
          {[1,2,3,4,5].map(i => (
            <svg key={i} className="w-3 h-3" fill={i <= 4 ? '#F59E0B' : '#E5D8C0'} viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
          ))}
          <span className="text-[10px] font-sans ml-1" style={{ color: '#9a8878' }}>4.8 (124)</span>
        </div>

        <div className="flex items-baseline gap-2">
          <span className="font-sans font-bold text-xl" style={{ color: isOutOfStock ? '#9a8878' : '#2C2420' }}>
            {product.retailPriceEur} €
          </span>
          {product.compareAtPriceEur && (
            <span className="text-sm line-through" style={{ color: '#9a8878' }}>{product.compareAtPriceEur} €</span>
          )}
        </div>
      </div>
    </div>
